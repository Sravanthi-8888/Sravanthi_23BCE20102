...
#22 may
use tuesday;
alter table worker add column dept varchar(25);
update worker set dept='admin' where salary>6000;
update worker set dept='accountant' where salary<=6000 and salary>30000;
update worker set dept='intern' where  salary<=30000;


select * from worker;
select count(dept),dept  from worker group by dept ;
select count(dept),dept  from worker where dept='admin' group by dept ;
select max(salary),dept  from worker group by dept;
select sum(salary),dept from worker group by dept order by sum(salary) asc limit 2 offset 1 ; 
select sum(salary),dept from worker group by dept having count(dept)>1;
select min(salary) as m,dept from worker group by dept;
...
insert into students values (101,'jaya'),(102,'sri'),(103,'latha'),(104,'rama'),(105,'sita');
drop table address;
create table address(
s_id int,
addr varchar(25));
insert into address values (101,'sangadigunta'),(102,'lr colony'),(103,'anand peta'),(104,'lalapeta'),(105,'arandalpeta');
select * from address cross join students;
select * from address join students;
select * from address inner join students where address.s_id=students.s_id;
select * from address left outer join students on address.s_id=students.s_id;
select * from address right outer join students on address.s_id=students.s_id;
select name1,salary from worker w1 where 4=(select count(distinct salary) from worker w2 where w2.salary>w1.salary);